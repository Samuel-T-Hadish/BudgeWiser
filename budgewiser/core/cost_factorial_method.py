...
def select_method():
    ...
    method_choice = inquirer.select(
        message="Choose method:", choices=method_choices
    ).execute()
...
def select_plant_type(method_choice):
    ...
    """
    plant_choices = (
        df[df["method"] == method_choice]["plant_type"].dropna().unique().tolist()
    )
    plant_choice = inquirer.select(
        message="Choose plant type:", choices=plant_choices
    ).execute()
...
def select_equipment(plant_choice):
    ...
    """
    equipment_choices = (
        df[df["plant_type"] == plant_choice]["equipment"].dropna().unique().tolist()
    )
    selected_equipment = inquirer.select(
        message="Choose equipment type:", choices=equipment_choices
    ).execute()
...
def select_equipment_type(selected_equipment, method_choice, plant_choice):
    ...
    type_choice = inquirer.select(
        message="Choose specific type for the equipment:", choices=type_choices
    ).execute()
    ...
    return {
        ...
        "instrumentation_and_control_factor": selected_row[
            "instrumentation_and_control_factor"
        ],
        "electrical_factor": selected_row["electrical_factor"],
        "civil_factor": selected_row["civil_factor"],
        "structures_and_buildings_factor": selected_row[
            "structures_and_buildings_factor"
        ],
...
s = get_valid_sizing_input(
    type_info["s_lower"],
    type_info["s_upper"],
    type_info["sizing_quantity"],
    type_info["units"],
)
Purchased_equipment_cost = purchased_equipment_cost(
    type_info["a"], type_info["b"], s, type_info["n"]
)
...
if method_choice == "Hand":
    Installed_equipment_cost = installed_equipment_cost(
        type_info["installation_factor"], Purchased_equipment_cost
    )
    ...
    ISBL_cost, Total_fixed_capital_cost = total_fixed_capital_cost(
        type_info["material_factor"],
        type_info["equipment_erection_factor"],
        type_info["piping_factor"],
        type_info["instrumentation_and_control_factor"],
        type_info["electrical_factor"],
        type_info["civil_factor"],
        type_info["structures_and_buildings_factor"],
        type_info["lagging_and_paint_factor"],
        type_info["Offsites_factor"],
        type_info["design_and_engineering_factor"],
        type_info["contingency"],
        type_info["location_factor"],
        Purchased_equipment_cost,
    )
    ISBL_cost = ISBL_cost
